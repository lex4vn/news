...
if( ! nv_function_exists( 'nv_news_block_topline' ) )
{
	...
	function nv_news_block_topline( $block_config, $mod_data )
	{
		...
		$cache_file = NV_LANG_DATA . '__block_topline_' . $numrow . '_' . NV_CACHE_PREFIX . '.cache';
		if( ( $cache = nv_get_cache( $module, $cache_file ) ) != false )
		{
			$array_block_topline = unserialize( $cache );
		}
		else
		{
			$array_block_topline = array();
			...
			$cache = serialize( $array_block_topline );
			nv_set_cache( $module, $cache_file, $cache );
		}

		if( file_exists( NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/news/block_topline.tpl' ) )
		{
		...
		$xtpl = new XTemplate( 'block_topline.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news/' );

		foreach( $array_block_topline as $array_news )
		{
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	if( isset( $site_mods[$module] ) )
	{
		...
		$content = nv_news_block_topline( $block_config, $mod_data );
	}
}